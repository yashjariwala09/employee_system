...
app.post("/user/register",(req,res)=>{
    let user=Imports.lodash.pick(req.body,['empid','name','gender',
                                            'dob','doJoin','qulification',
                                            'exprience','Phone_no','email',
                                            'address','addedBy','isActive']);

    Imports.User.addEmployee(user).then(()=>res.send()).catch(()=>res.status(404).send());
});
